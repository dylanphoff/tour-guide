from tour_stop.tour_stop_activity import TourStopActivity, TourStopActivityType, TriviaActivity
...
class TourStopActivityExecutor():
    def execute_activity(self, activity: TourStopActivity) -> None:
        ...
        raise ValueError(f"No tour stop activity matching {activity.get_type()}")
    ...
    def _execute_trivia_activity(self, activity: TriviaActivity) -> None:
        ...
        if ord(answer) == ord('a') + activity.correct_answer_index:
            ...
            correct_answer = bulleted_answer_options[activity.correct_answer_index]
